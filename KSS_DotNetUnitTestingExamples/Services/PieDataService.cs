...
{
    /// <summary>
    /// Pie Data service that accepts Date time from the business layer
    /// </summary>
    public interface IPieDataService
    {
        Task<Pie> BakePie(string flavour, int pastry, int filling);
    ...
    public class PieDataService : IPieDataService
    {
        ...
        public async Task<Pie> BakePie(string flavour, int pastry, int filling)
        {
            Pie pie = await CookPie(flavour, pastry, filling);

            await UpdatePieAudit(flavour);
            return pie;
        }
        ...
        public async Task UpdatePieAudit(string flavour)
        {
            List<Pie> pies = new List<Pie> { new Pie { Flavour = flavour, LastMadeOn = DateTime.Now.ToString("T") } };
        ...
        private async Task<Pie> CookPie(string flavour, int pastry, int filling)
        {
            Pie pie = new Pie
            {
                Flavour = flavour,
